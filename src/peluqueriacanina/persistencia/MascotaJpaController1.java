...
public class MascotaJpaController1 implements Serializable {
    ...
    public void destroy(int num_cliente) throws NonexistentEntityException {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Mascota mascota;
            try {
                mascota = em.getReference(Mascota.class, num_cliente);
                mascota.getNum_cliente();
            } catch (EntityNotFoundException enfe) {
                throw new NonexistentEntityException("The mascota with id " + num_cliente + " no longer exists.", enfe);
            }
            em.remove(mascota);
            em.getTransaction().commit();
        } finally {
